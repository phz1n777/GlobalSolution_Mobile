...
export default function ProfileScreen({ navigation }: any) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setUser(u);
      if (u) { setName(u.username); setArea(u.profile?.area || 'Produtividade'); }
    })();
  }, []);
  ...
  if (!user) return <SafeAreaView style={styles.center}><ActivityIndicator /></SafeAreaView>;

  return (
    <SafeAreaView style={[styles.container, focusOn && styles.focusModeBackground]}>
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={[styles.headerSmall, focusOn && styles.focusModeCard]}>
        ...
        <View style={[styles.card]}>
          <View style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
            ...
            <View style={{ flex: 1 }}>
              {!editing ? <>
                <Text style={{ fontWeight: '900', fontSize: 18, color: PALETTE.textDark }}>{user.username}</Text>
                <Text style={{ color: PALETTE.textMuted, marginTop: 6 }}>{user.email}</Text>
                <Text style={{ color: PALETTE.textMuted, marginTop: 6 }}>Área: {user.profile?.area || '—'}</Text>
              </> : <>
                ...
                <View style={styles.pickerWrap}>
                  <Picker selectedValue={area} onValueChange={(v)=>setArea(v)} style={{
                color: PALETTE.textDark,        // cor do texto do item selecionado
                backgroundColor: '#fff',        // força fundo branco no iOS
                width: '100%',
              }}
              itemStyle={{
                color: PALETTE.textDark,        // cor dos itens no iOS (wheel)
                fontSize: 16,
              }}
              dropdownIconColor={PALETTE.textDark} // android → seta preta
            >
          ...
          <View style={{ marginTop: 12 }}>
            {!editing ? (
              ...
              <>
                ...
                <TouchableOpacity style={[styles.ghostBtn, { marginTop: 8 }]} onPress={() => { setEditing(false); }}>
